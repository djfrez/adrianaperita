...
- ~~**Manutenção factual pendente:** `/pericia-contaminacao-alimentos/` descreve a CP 1.362/2025 como "em andamento".~~ **Corrigida em 19/08**, a pedido da cliente e sem esperar o SEO-026: o bloco agora diz que a consulta foi publicada em 11/12/2025, que o prazo de contribuições se encerrou e que a norma final não saiu — e que, até que saia, a exigência aferível é a da RDC 275/2002, porque consulta pública não vincula. Ganhou link contextual para `/normas-tecnicas-pericia/#rdc-275`. `dateModified` movido para 19/08 (correção substantiva) e `lastmod` idem. **Efeito colateral declarado:** a página é uma das três do SEO-026 e recebeu hoje uma aresta de entrada — a atribuição do SEO-026 nessa página passa a competir com esta mudança, e a leitura precisa considerar as duas.
...
- **Renderização conferida — mas só depois de publicar, e isso muda o processo.** A verificação local falhou duas vezes: o servidor HTTP local não subiu (porta bloqueada no ambiente) e o `file://` renderizou como snapshot estático com `innerWidth` 0 — em que **todo** elemento aparece como estourando a janela, uma medida que teria sido lida como catástrofe se levada a sério. A checagem foi então feita **contra a URL publicada**, depois do push, com o viewport fixado explicitamente: a 1280 px e a 375 px o **overflow do documento é 0** e não há nenhum elemento novo excedendo a janela; as duas tabelas pré-existentes seguem rolando dentro do próprio `.table-scroll` (`clientWidth` 311 contra `scrollWidth` 544, idêntico às medições de 16/08 e 18/08); e `scroll-margin-top` resolve em 96 px nas cinco âncoras novas. Antes disso, a triagem estrutural: bloco novo só com `h2`, `h3`, `p`, `ol`, `li`, `strong`, `em`, `a` e `div.box`, **zero tabelas** e **zero tokens acima de 28 caracteres** — as duas causas reais de overflow neste layout.
- **Lição de processo:** `innerWidth` 0 significa "sem viewport", não "layout quebrado". **Fixar a largura explicitamente e conferir que `innerWidth` bateu antes de ler qualquer medida de overflow** — sem isso a medição é ruído com aparência de dado.
