...
### SEO-034 — Página-pilar: o laudo pericial como documento *(executada em 2026-08-22)*

- **Descrição:** criar a página que faltava entre "formular quesitos" e "impugnar o laudo" — o que o art. 473 do CPC obriga o laudo a conter, o que veda ao perito, um roteiro de conferência de 22 pontos que cabe nos 15 dias do art. 477, e a tabela que liga cada defeito encontrado à via processual correta.
- **URL:** `/laudo-pericial/`
- **Categoria:** Conteúdo / Autoridade tópica / Cobertura de intenção procedimental / AI citation
- **Impacto:** 8 · **Esforço:** 5 · **Confiança:** 8 · **Valor de negócio:** 9
- **Priority Score:** 115,2
- **Status:** done · **Descoberto:** 2026-08-22 · **Concluído:** 2026-08-22

**Por que esta, e não a réplica do SEO-033.** A série do GSC de hoje vai até **20/08** — o SEO-033 foi publicado em **21/08** e tem **zero dias de dado**. Ler o experimento hoje seria inventar resultado. E os dados de hoje repetem, mais forte, a leitura que originou o SEO-033: **as seis páginas procedimentais carregam 253 das 294 impressões; as seis páginas de matéria técnica carregam 28 somadas.** Toda página procedimental que o site publicou ganhou exposição; nenhuma página de matéria ganhou. A conclusão operacional é publicar **o próximo nó procedimental**, não replicar a porta procedimental em mais uma página de matéria — isso continua sendo o teste do SEO-033 e depende da leitura de ~28/08.

**A lacuna era literal.** O site ensinava a *perguntar* (`/quesitos-periciais/`, 103 imp) e a *atacar* (`/impugnacao-laudo-pericial/`, 72 imp), e não tinha uma linha sobre **o documento que chega entre as duas coisas**. O art. 473 existia como um `h3` de uma página de referência organizada por artigo (`/cpc-prova-pericial/`), consultada por quem já sabe o número do dispositivo. Quem tem o laudo na mão e quinze dias correndo não procura "art. 473" — procura "o que o laudo pericial deve conter" e "como analisar um laudo pericial".

**O ativo é o roteiro de 22 pontos, e ele é utilizável como está.** Seis blocos com âncora própria: `#roteiro-identificacao` (4 pontos — assinatura do próprio nomeado, registro ativo, área compatível com o objeto, e os nomes dos profissionais quando a perícia é de órgão técnico, pelo art. 156, §4º), `#roteiro-objeto` (3), `#roteiro-metodo` (6), `#roteiro-dados` (4), `#roteiro-quesitos` (3) e `#roteiro-conclusao` (2). Os pontos que exigem conhecimento da matéria estão marcados `(técnico)` no próprio texto — são a demonstração honesta de onde o advogado precisa de assistente técnico, em vez de uma afirmação genérica de que precisa.

**Três precisões que separam a página do conteúdo concorrente:**
1. **O inciso III são três exigências, não uma** — indicar o método, esclarecê-lo *e demonstrar que é predominantemente aceito pelos especialistas da área*. A terceira é a mais omitida nos laudos e a mais ignorada nos guias.
2. **Resposta não conclusiva (inciso IV) e resposta conclusiva sem dado que a sustente (§1º) são defeitos distintos**, com redações de crítica diferentes. Confundir os dois é o que produz manifestação genérica.
3. **O §3º é faculdade, não obrigação.** Não existe dever expresso de anexar dado bruto — a ausência do dado se ataca pelo inciso III e pelo §1º. Dito ao contrário, como fazem vários guias, a crítica cai na primeira réplica.

**A tabela de encaminhamento é o que converte.** Sete linhas ligando *o que foi encontrado* → *o que se pede* → *fundamento*: quesito sem resposta → esclarecimentos (art. 477, §2º, I); método não demonstrado → esclarecimentos e, persistindo, parecer divergente; premissa técnica errada → parecer divergente no mesmo prazo, que aciona o dever de esclarecer do art. 477, §2º, II; juízo de culpa no texto → desconsideração do trecho (art. 473, §2º); **perito sem conhecimento na área → substituição pelo art. 468, I, e não crítica ao resultado**; matéria ainda mal esclarecida → nova perícia (art. 480).

**"Três leituras que parecem defeito e não são"** existe para proteger o prazo do leitor: laudo desfavorável mas metodologicamente íntegro, divergência legítima de interpretação apresentada como erro, e não resposta a quesito que o perito estava **legalmente impedido** de responder. É a seção que um concorrente que escreve para vender impugnação não escreve.

**Não canibaliza `/impugnacao-laudo-pericial/`, e a divisão é a mesma do SEO-033.** Aquela página decide **qual via usar** e ataca **o método** (as seis falhas); esta trata **do documento** — o que a lei exige que ele contenha e como conferir. Triagem antes de decisão. **`/impugnacao-laudo-pericial/` não foi tocada** (está em melhora sustentada e a orientação era não mexer); a nova página aponta para ela três vezes, em mão única.

**Verificação factual — texto literal conferido na fonte, não de memória.** Baixado o texto compilado da Lei nº 13.105/2015 no Planalto e conferidos palavra a palavra: **art. 473, incisos I a IV e §§ 1º a 3º**; **art. 477, caput e §§ 1º e 2º**; **art. 468, I**; **art. 156, §§ 1º, 4º e 5º**; **art. 157, §1º**; **arts. 479 e 480, §§ 1º a 3º**. Duas correções que a conferência produziu: o inciso IV fala em quesitos do juiz, das partes e **"do órgão do Ministério Público"**; e o **§3º é redigido como faculdade** ("podem valer-se… bem como instruir o laudo com…"), o que mudou a redação da FAQ 7 — a resposta afirma que **não há obrigação expressa** e desloca o fundamento do pedido para o inciso III e o §1º.

**Links de entrada: 4, todos aditivos de uma linha.** Card novo na seção "Guias técnicos" da home (inserido **antes** do card de impugnação, para que a ordem de leitura siga o processo) e um item no bloco "relacionados" de `/cpc-prova-pericial/`, `/quesitos-periciais/` (apontando direto para `#roteiro-quesitos`) e `/assistente-tecnica/`. **`/pericia-industria-quimica/`, `/pericia-contaminacao-alimentos/`, `/pericia-ambiental/` e `/impugnacao-laudo-pericial/` não foram tocadas** — as três primeiras são o controle e o experimento do SEO-026/033, a quarta está sob "não mexer".

**Schema:** `Article` + `FAQPage` (9 entradas) + `BreadcrumbList`, com o `@id` canônico do `Person` do site. `about` com `Laudo Pericial`, `Prova Pericial`, `Quesitos Periciais` e `Assistente Técnico`. A home ganhou um `Offer` no `hasOfferCatalog` — "Análise Crítica de Laudo Pericial" — apontando para a página nova.

**Dois defeitos de renderização encontrados e corrigidos antes do commit, ambos invisíveis para o validador:**
1. **A folha de estilo compartilhada anula o atributo `start` das listas.** A regra é `ol { counter-reset: n }` com `ol li::before { content: counter(n) "." }` — o contador reinicia em toda `<ol>`, então os blocos 5–7, 8–13, 14–17, 18–20 e 21–22 renderizariam todos como 1, 2, 3. Corrigido com `style="counter-reset: n N"` em cada lista, mantendo o `start` para semântica e leitor de tela. **Vale para qualquer página futura deste site que use lista numerada contínua em mais de um bloco.**
2. **A página nasceu sem `h2[id], h3[id] { scroll-margin-top: 6rem }`** — o CSS foi clonado de `/cpc-prova-pericial/`, que só tem a regra dentro de `.art-entry`. Com 12 âncoras e seis delas citadas no `llms.txt`, o link direto esconderia o título sob o cabeçalho fixo. Regra adicionada.

**Validação executada:** `[deploy]` sem deriva antes da mudança (0 commits pendentes, 14 URLs em 200) · `[valid]` **ALL PASS nas 15 páginas** · balanceamento de tags por `html.parser` nos 5 arquivos tocados: nenhuma pendência · **paridade FAQ 9/9, comparação caractere a caractere de pergunta E resposta** · contagem dos 22 pontos conferida por script contra os `start` das listas (4+3+6+4+3+2) · zero token acima de 28 caracteres no texto visível (os que o script aponta estão todos dentro de `<script>`) · âncoras `#prazos` e `#roteiro-quesitos` conferidas nos dois destinos. Título 49 caracteres, descrição 154.
**Renderização conferida contra a URL publicada, e ela reprovou duas vezes antes de passar.** A conferência a 375 px encontrou um defeito real que nenhuma validação de arquivo pegaria, e o método que o revelou é a parte reaproveitável:

- **A regra do SEO-033 — "`innerWidth` ≠ `clientWidth` significa medida inválida" — precisou de um complemento para virar diagnóstico.** Sozinha, ela apenas manda descartar a leitura. O que transformou o descarte em achado foi **medir uma página boa conhecida na mesma aba, no mesmo instante**: `/cpc-prova-pericial/` assentou em 375 e reportou overflow 0, enquanto `/laudo-pericial/` insistia em `innerWidth` 447. **Duas páginas, mesma janela, resultados diferentes ⇒ a diferença é da página, não do ambiente.** Sem o controle, o 447 teria sido descartado como ruído — e o defeito teria sido publicado.
- **O defeito: `li { display: flex }` transforma cada filho inline em um item de flex numa linha que não quebra.** Não é só `<a>` — `<em>` e `<strong>` também. Os itens do roteiro que começam com `<em>(técnico)</em>` estouravam a própria caixa (`scrollWidth` 415 contra `clientWidth` 311) e empurravam o documento para 447 de largura no celular.
- **A primeira correção foi estreita demais e não bastou.** Envolveu em `<span>` apenas os dois itens com link inline; os elementos com `<em>` continuaram estourando. Só a segunda correção — **envolver o conteúdo dos 29 itens de lista**, deixando exatamente dois itens de flex por linha (marcador e conteúdo) — zerou a medida. **Corrigir o caso que a medição apontou, em vez da causa que ela revelou, custou um ciclo inteiro de commit, push e espera de deploy.**
- **Medida final, com o viewport assentado (`innerWidth` = `clientWidth` = 375):** overflow do documento **0**, **zero elementos** excedendo a janela, zero caixas com `scrollWidth` maior que o próprio `clientWidth`, e as duas tabelas rolando dentro do `.table-scroll` (311 contra 544, idêntico a todas as outras páginas). A 1280 px: overflow 0, tabelas cabendo inteiras (832/832), `scroll-margin-top` em 96 px nas 12 âncoras.
- **A numeração contínua foi verificada empiricamente, não presumida.** `getComputedStyle` devolve `counter(n) "."` sem resolver o contador, então a checagem foi indireta: com `font-variant-numeric: tabular-nums`, um marcador de dois dígitos é mensuravelmente mais largo que um de um dígito. As larguras medidas — 32 px nos itens 1 a 9 e 43 px nos itens 10 a 22 — confirmam que os blocos pintam 5–7, 8–13, 14–17, 18–20 e 21–22, e não 1, 2, 3 em cada bloco.
...
- **Nenhuma delas estoura hoje** — `/cpc-prova-pericial/` foi medida a 375 px nesta execução e assentou com overflow 0. É risco latente, não defeito ativo: qualquer edição que alongue um desses itens o dispara em silêncio.
