...
- **GA4 contradiz o "0 cliques" do Search Console — e é o achado mais importante do dia.** 42 sessões em 28 dias, das quais **4 de busca orgânica**, em quatro páginas distintas: `/assistente-tecnica/`, `/quesitos-periciais/`, `/pericia-industria-quimica/` e `/impugnacao-laudo-pericial/`. O Search Console reporta 0 clique no mesmo período. As duas leituras são conciliáveis por duas vias, e vale distinguir qual é antes de concluir qualquer coisa sobre CTR: (a) o "Organic Search" do GA4 inclui Bing, DuckDuckGo e outros, enquanto o GSC só mede Google; (b) latência do GSC nos últimos dias da janela. **Consequência prática:** as páginas já recebem visita orgânica, o que enfraquece a hipótese de que o problema seja snippet e reforça que o gargalo é alcance. Também aparecem **8 sessões de Paid Search na home**, do Google Ads.
  - **A checar na próxima execução:** cruzar `sessionSource` no GA4 para separar Google de Bing. Se as 4 sessões orgânicas forem de Bing, o site converte impressão em clique fora do Google e o diagnóstico do Google fica isolado; se forem do Google, o "0 cliques" do GSC é só latência e a régua de 16/08 precisa ser relida com dados já disponíveis.
...
2. **Separar Google de Bing nas 4 sessões orgânicas do GA4** (dimensão `sessionSource`). É a checagem mais barata da lista e determina se o "0 cliques" do Search Console é latência ou é real — ou seja, se a régua do item 3 ainda vale.
3. **CTR das páginas entre 5 e 12 — só a partir de ~16/08.** A janela de recache dos snippets abriu em 09/08. Antes disso nenhuma conclusão sobre metadado é válida, e com menos de ~200 impressões acumuladas 0 clique continua sendo ruído, não diagnóstico.
4. **`/impugnacao-laudo-pericial/` — reavaliar por volta de 20/08**, quando tiver duas semanas ao vivo. É onde está a demanda real (4 das 8 consultas) e a pior posição do site. Se seguir acima de 25, tratar como defeito da página, não como idade.
5. **`/pericia-ambiental/` e `/pericia-contaminacao-alimentos/`: zero impressão em 28 dias, apesar de indexadas.** Duas páginas indexadas que nunca apareceram é o achado mais concreto para investigar depois do item 4 — provável descasamento entre o que a página responde e o que se pesquisa nesses dois temas.
6. **Só depois disso**, a próxima spoke por intenção continua sendo **recolhimento/recall pela RDC 655/2022**, que exigirá encurtar a seção correspondente do pilar de alimentos.
